...
# Panel A: Supervisor's Style (Sorted by Precision)
s <- read_csv("figdata/fig7_consensus_scores.csv", show_col_types = FALSE) %>%
  # مرتب‌سازی مدل‌ها بر اساس افزایش Precision
  mutate(rule = fct_reorder(rule, precision)) %>%
...
save_fig(p, "fig7_consensus_supervisor3", width = 8.5, height = 9)
