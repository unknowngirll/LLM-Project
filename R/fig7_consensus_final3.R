...
# Panel A: Trend Lines for Metrics (matching Panel B's style)
...
a <- ggplot(s, aes(x = rule, y = value, color = metric, group = metric)) +
  ...
  labs(x = NULL, y = "Score", tag = "A", color = NULL) +
  theme_report() +
  theme(
    legend.position = "top",
...
save_fig(p, "fig7_consensus_final3", width = 8.5, height = 9)
