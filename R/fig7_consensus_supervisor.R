...
# Panel A: Supervisor's Style (Grouped Bars with Text Labels & Clean Theme)
...
a <- ggplot(s, aes(x = rule, y = value, fill = metric)) +
  ...
  # چاپ اعداد دقیق روی میله‌ها (ویژگی بارز مقالات)
  geom_text(aes(label = sprintf("%.3f", value)),
            position = position_dodge(width = 0.8),
            vjust = -0.5, size = 3, color = "black") +
  # پالت رنگی کلاسیک مقالات
  ...
  # استفاده از تم کلاسیک برای حذف خطوط اضافی پس‌زمینه
  theme_classic() +
  theme(
    legend.position = "top",
...
save_fig(p, "fig7_consensus_supervisor", width = 8.5, height = 9)
