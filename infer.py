#!/usr/bin/env python3
"""
infer.py - run one model over the eval dataset, save raw outputs.
Path B: plain transformers, no unsloth.
"""
...
for i, ex in enumerate(ds):
    ...
    dt = time.time() - t0
    with open(os.path.join(out_dir, f"{pmid}.txt"), "w", encoding="utf-8") as f:
        f.write(text)
