#!/usr/bin/env python3
"""
infer_think_seed.py - thinking-mode inference with a controllable seed.
Same as infer_think_simple.py plus:
  - --seed, applied per paper (seed*100000 + index) so runs stay
    reproducible and resumable after a timeout
...
log(f"Running on {len(ds)} records. temp={args.temperature} seed={args.seed} "
...
for i, ex in enumerate(ds):
    ...
    # per-paper seed: reproducible and unaffected by resume order
    torch.manual_seed(args.seed * 100000 + i)
    torch.cuda.manual_seed_all(args.seed * 100000 + i)

    kw = dict(max_new_tokens=args.max_new_tokens, pad_token_id=PAD)
